...
type UmkmRepository interface {
	CreateUmkm(umkm *domain.Umkm) error
	FindUmkm(
		search string,
		page int,
		limit int,
	...
	FindUmkmByID(id uint) (*domain.Umkm, error)
	CountStatus() (verified int64, unverified int64, err error)
	FindUmkmBySlug(slug string) (*domain.Umkm, error)
	UpdateUmkm(umkm *domain.Umkm) error
	DeleteUmkm(umkm *domain.Umkm) error
}
...
func (r *umkmRepository) CreateUmkm(umkm *domain.Umkm) error {
...
func countByStatus(db *gorm.DB, status string) (int64, error) {
	...
	err := db.Model(&domain.Umkm{}).Where("status = ?", status).Count(&count).Error
	return count, err
}
...
func (r *umkmRepository) FindUmkmByID(id uint) (*domain.Umkm, error) {
	var umkm domain.Umkm
	err := r.db.Find(&umkm, id).Error
	return &umkm, err
}

func (r *umkmRepository) FindUmkmBySlug(slug string) (*domain.Umkm, error) {
	var umkm domain.Umkm
...
func (r *umkmRepository) UpdateUmkm(umkm *domain.Umkm) error {
	return r.db.Save(umkm).Error
}

func (r *umkmRepository) DeleteUmkm(umkm *domain.Umkm) error {
