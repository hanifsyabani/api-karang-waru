package handler

import (
	"api-karang-waru/pkg/types"
	"api-karang-waru/internal/usecase"
	"net/http"
	"strconv"

	"github.com/gin-gonic/gin"
)

type UmkmHandler struct {
	service usecase.UmkmService
}

func NewUmkmHandler(service usecase.UmkmService) *UmkmHandler {
	return &UmkmHandler{service}
}

func (h *UmkmHandler) CreateUmkm(c *gin.Context) {
	// UmkmRequest → fokus pada data yang dikirim umkm.
	// domain.Umkm → fokus pada struktur tabel di database.

	var req types.UmkmRequest

	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, types.APIResponse{
			Code:    "BAD_REQUEST",
			Message: err.Error(),
			Data:    nil,
		})
		return
	}

	umkm, err := h.service.CreateUmkm(&req)

	if err != nil {
		c.JSON(http.StatusInternalServerError, types.APIResponse{
			Code:    "INTERNAL_SERVER_ERROR",
			Message: err.Error(),
			Data:    nil,
		})
		return
	}

	// ketika succes
	c.JSON(http.StatusCreated, types.APIResponse{
		Code:    "CREATED",
		Message: "Umkm created successfully",
		Data:    types.UmkmResponseFromModel(umkm),
	})
}

// get all
func (h *UmkmHandler) GetAllUmkm(c *gin.Context) {
	search := c.Query("search")
	status := c.Query("status")
	page, _ := strconv.Atoi(c.DefaultQuery("page", "1"))
	limit,_ := strconv.Atoi(c.DefaultQuery("limit", "10"))
	sortBy := c.DefaultQuery("sortBy", "created_at")
	sortOrder := c.DefaultQuery("sortOrder", "desc")
	umkm, err := h.service.GetAllUmkm(
		search,
		page,
		limit,
		sortBy,
		sortOrder,
		status,
	)

	if err != nil {
		c.JSON(http.StatusInternalServerError, types.APIResponse{
			Code:    "INTERNAL_SERVER_ERROR",
			Message: err.Error(),
			Data:    nil,
		})
		return
	}

	var UmkmResponse []types.UmkmResponse
	for _, umkm := range umkm {
		UmkmResponse = append(UmkmResponse, types.UmkmResponseFromModel(&umkm))
	}

	c.JSON(http.StatusOK, types.APIResponse{
		Code:    "OK",
		Message: "Umkm retrieved successfully",
		Data:    UmkmResponse,
	})

}

func (h *UmkmHandler) GetCountStatus(c *gin.Context) {
	verified, unverified, err := h.service.GetCountStatus()
	if err != nil {
		c.JSON(http.StatusNotFound, types.APIResponse{
			Code:    "NOT_FOUND",
			Message: err.Error(),
			Data:    nil,
		})
		return
	}

	c.JSON(http.StatusOK, types.APIResponse{
		Code:    "OK",
		Message: "umkm retrieved successfully",
		Data:    gin.H{
			"verified":   verified,
			"unverified": unverified,
		},
	})
}

// get by id
func (h *UmkmHandler) GetUmkmByID(c *gin.Context) {
	idParam := c.Param("id")
	id, err := strconv.Atoi(idParam)
	if err != nil {
		c.JSON(http.StatusBadRequest, types.APIResponse{
			Code:    "BAD_REQUEST",
			Message: "Invalid umkm ID",
			Data:    nil,
		})
		return
	}

	umkm, err := h.service.GetUmkmByID(uint(id))
	if err != nil {
		c.JSON(http.StatusNotFound, types.APIResponse{
			Code:    "NOT_FOUND",
			Message: err.Error(),
			Data:    nil,
		})
		return
	}

	c.JSON(http.StatusOK, types.APIResponse{
		Code:    "OK",
		Message: "umkm retrieved successfully",
		Data:    types.UmkmResponseFromModel(umkm),
	})
}

func (h *UmkmHandler) GetUmkmBySlug(c *gin.Context) {
	slug := c.Param("slug")
	umkm, err := h.service.GetUmkmBySlug(slug)
	if err != nil {
		c.JSON(http.StatusNotFound, types.APIResponse{
			Code:    "NOT_FOUND",
			Message: err.Error(),
			Data:    nil,
		})
		return
	}

	c.JSON(http.StatusOK, types.APIResponse{
		Code:    "OK",
		Message: "umkm retrieved successfully",
		Data:    types.UmkmResponseFromModel(umkm),
	})
}

// update
func (h *UmkmHandler) UpdateUmkm(c *gin.Context) {
	idParam := c.Param("id")
	id, err := strconv.Atoi(idParam)
	if err != nil {
		c.JSON(http.StatusBadRequest, types.APIResponse{
			Code:    "BAD_REQUEST",
			Message: "Invalid umkm ID",
			Data:    nil,
		})
		return
	}

	var req types.UmkmRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, types.APIResponse{
			Code:    "BAD_REQUEST",
			Message: err.Error(),
			Data:    nil,
		})
		return
	}

	umkm, err := h.service.UpdateUmkm(uint(id), &req)
	if err != nil {
		c.JSON(http.StatusInternalServerError, types.APIResponse{
			Code:    "INTERNAL_SERVER_ERROR",
			Message: err.Error(),
			Data:    nil,
		})
		return
	}

	c.JSON(http.StatusOK, types.APIResponse{
		Code:    "OK",
		Message: "Umkm updated successfully",
		Data:    types.UmkmResponseFromModel(umkm),
	})
}

// delete
func (h *UmkmHandler) DeleteUmkm(c *gin.Context) {
	idParam := c.Param("id")
	id, err := strconv.Atoi(idParam)
	if err != nil {
		c.JSON(http.StatusBadRequest, types.APIResponse{
			Code:    "BAD_REQUEST",
			Message: "Invalid umkm ID",
			Data:    nil,
		})
		return
	}

	err = h.service.DeleteUmkm(uint(id))
	if err != nil {
		c.JSON(http.StatusInternalServerError, types.APIResponse{
			Code:    "INTERNAL_SERVER_ERROR",
			Message: err.Error(),
			Data:    nil,
		})
		return
	}

	c.JSON(http.StatusOK, types.APIResponse{
		Code:    "OK",
		Message: "Umkm deleted successfully",
		Data:    nil,
	})
}

